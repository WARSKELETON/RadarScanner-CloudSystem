...
declare -a StringArray=("ICount" "DynICountOpt")
...
for val in ${StringArray[@]}; do
  ...
  for view in 64 128 256 512 1024 2048; do
    echo $view
    time java pt.ulisboa.tecnico.cnv.solver.SolverMain -d -s GRID_SCAN -w 2048 -h 2048 -x0 0 -x1 $((view-1)) -y0 0 -y1 $((view-1)) -i 'datasets/SIMPLE_VORONOI_2048x2048_1.png' -yS $((view/2)) -xS $((view/2)) > results2048/"$val""$view"_GS.txt
    time java pt.ulisboa.tecnico.cnv.solver.SolverMain -d -s PROGRESSIVE_SCAN -w 2048 -h 2048 -x0 0 -x1 $((view-1)) -y0 0 -y1 $((view-1)) -i 'datasets/SIMPLE_VORONOI_2048x2048_1.png' -yS $((view/2)) -xS $((view/2)) > results2048/"$val""$view"_PS.txt
    time java pt.ulisboa.tecnico.cnv.solver.SolverMain -d -s GREEDY_RANGE_SCAN -w 2048 -h 2048 -x0 0 -x1 $((view-1)) -y0 0 -y1 $((view-1)) -i 'datasets/SIMPLE_VORONOI_2048x2048_1.png' -yS $((view/2)) -xS $((view/2)) > results2048/"$val""$view"_GRS.txt
  done
done